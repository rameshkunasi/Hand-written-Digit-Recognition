...
validation_set = validation_set.reshape(validation_set.shape[0], 28, 28)
...
trainDataset = TensorDataset(torch.from_numpy(train_set), torch.from_numpy(trainLabel2))
validDataset = TensorDataset(torch.from_numpy(validation_set), torch.from_numpy(validLabel2))
# Creating dataloader
trainLoader = DataLoader(trainDataset, batch_size=64, shuffle=True,num_workers=4, pin_memory=True)
validtLoader = DataLoader(validDataset, batch_size=64, shuffle=True,num_workers=4, pin_memory=True)
...
model = train_model(model, criterion, num_epochs, learning_rate, trainLoader, use_gpu)
